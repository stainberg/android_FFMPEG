...
typedef struct HLSContext {
    ...
    double target_duration;//Mod By Yen
    int start_seq_no;
    int finished;
    int n_segments;
...
static int parse_playlist(URLContext *h, const char *url)
{
    ...
    double duration = 0.0;//Mod By Yen
    char line[1024];
    const char *ptr;
    ...
    while (!url_feof(in)) {
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            ...
            s->target_duration = atof(ptr);//Mod By Yen
