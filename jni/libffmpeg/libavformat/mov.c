...
#if 1 // added by mabiao for drm
#endif
#if _SUPPORT_SUMA_DRM
#include "../../include/drm4ffmpeg.h"
#endif
#if CONFIG_ZLIB
#include <zlib.h>
#endif
...
#if _SUPPORT_SUMA_DRM //added by mabiao for sinf atom
static const MOVParseTableEntry mov_default_parse_table_oma[]; 
static const MOVParseTableEntry *TableEntry;
static int mybool = 0;
#endif
...
static int mov_read_default(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    {
    	mybool = 1;
    	TableEntry = mov_default_parse_table;
    }
#endif
...
#if _SUPPORT_SUMA_DRM // Modified by  for taking sample 
		for (i = 0; (TableEntry+i)->type; i++)
			if ((TableEntry+i)->type == a.type) {
				parse = (TableEntry+i)->parse;
				break;
			}
#else
...
#if _SUPPORT_SUMA_DRM	// added by mabiao for sinf atom
    if (0 == strcmp(comp_brands_str, "mp41opf2"))
    {
    	//&mov_default_parse_table = &mov_default_parse_table_oma;
    	TableEntry = mov_default_parse_table_oma;
    }
#endif
...
#if _DRM_TEST
static unsigned int suma_drm_process_encbox(const char *encbox, size_t *boxlen, char *mp4box)
{
	return 0;
}

static void suma_drm_get_mp4info(char *szcontentid, char *szriurl, char *szdomainid, int *playable)
{
	char contentid[32] = "aaaaaaaaa";
	char url[256] = "bbbbbbbbb";
	char domainid[128] = "ccccccccc";
	int pable = 3;

	strcpy(szcontentid, contentid);
	strcpy(szriurl, url);
	strcpy(szdomainid, domainid);
	*playable = pable;
}

static unsigned int suma_drm_process_sample(char *sample, size_t *samplelen)
{
	return 3;
}

#endif
#if _DRM_TEST // Added by mabiao for drm log put out
static void suma_drm_logout(const char *file, const char *func, int line, int ret)
{
	//FILE *pfile = fopen("/data/ligenTest/ffmpeg_mb.log", "a");
	FILE *pfile = fopen("/mnt/sdcard/ligenTest/ffmpeg_mb.log", "a");
	if (pfile != NULL) {
		char *clog;

		fprintf(pfile, "%s:%s:%d:%d\n", file, func, line, ret);
		//av_log(pfile, "%s:%s:%d:%d\n", __FILE__, __func__, __LINE__, ret);
		fclose(pfile);
	}
...
#if _DRM_TEST //Added by mabiao for DRM notify test
    av_log(c->fc, AV_LOG_INFO, "mabiao test for drm info 1\n");	
#endif
    for (pseudo_stream_id=0; pseudo_stream_id<entries; pseudo_stream_id++) {
        //Parsing Sample description table
        enum CodecID id;
        int dref_id = 1;
#if _SUPPORT_SUMA_DRM //Added by mabiao for encv/encv
        const char* encbox;
		char* mp4box;// encv/enca and mp4v
        char* comp_mp4;
        AVIOContext enc_tmp_pb, mp4_tmp_pb;
        enc_tmp_pb = *pb; // copy struct AVIOContext
#endif
...
#if _SUPPORT_SUMA_DRM	//Added by mabiao for encv/encv
//        mp4_tmp_pb = *pb;
//        comp_mp4 = av_malloc(5);
//        avio_read(&mp4_tmp_pb, comp_mp4, 5);
//#if _DRM_TEST
//		av_log(c->fc, AV_LOG_INFO, "mabiao test for drm info 2\n");
//#endif
//        if(0 == strcmp(comp_mp4, "encv")||0 == strcmp(comp_mp4, "enca"))
//        {
//#if _DRM_TEST
//			av_log(c->fc, AV_LOG_INFO, "mabiao test for drm info 3\n");
//#endif
//        	size_t boxlen = size;//for drm process
//        	int encbox_size = size + 1;
//        	encbox = av_malloc(encbox_size);
//			mp4box = av_malloc(encbox_size);
//        	avio_read(&enc_tmp_pb, encbox, encbox_size);
//        	unsigned int ret = suma_drm_process_encbox(encbox, &boxlen, mp4box);
//#if _DRM_TEST
//			av_log(c->fc, AV_LOG_INFO, "mabiao test for drm info :encbox ret = %d\n", ret);
//#endif
//        	if(7 == ret) //drm order timeout
//        	{
//        		char contenid[256];
//        		char riurl[256];
//        		char domainid[256];
//        		int playble = 0;
//        		suma_drm_get_mp4info(contenid, riurl, domainid, &playble);
//
//#if _DRM_TEST
//				//FILE *pfile = fopen("/data/ligenTest/mp4info_mb.log", "a");
//				FILE *pfile = fopen("/mnt/sdcard/ligenTest/mp4info_mb.log", "a");
//        		if(pfile != NULL)
//        		{
//        			char *clog;
//
//        			fprintf(pfile, "%s:%s:%s:%d\n", contenid, riurl, domainid, playble);
//        			//av_log(pfile, "%s:%s:%d:%d\n", __FILE__, __func__, __LINE__, ret);
//        			fclose(pfile);
//        		}
//        		suma_drm_logout(__FILE__, __func__, __LINE__, ret);
//#endif
//				av_log(c->fc, AV_LOG_DRM, "mov.c:ret = 7:DRM info: %s:%s\n", contenid, riurl);	//Added by mabiao for DRM notify
//
//				av_freep(&encbox);
//				av_freep(&mp4box);
//
//        		return AVERROR_EOF; //stop play
//        		//return -7;
//
//        	}
//        	else if(0 != ret)
...
//        	}
//
//#if _DRM_TEST //Added by mabiao for encbox test
//        	FILE *fp;
//        	if((fp = fopen("/home/damon/aamm.txt", "w+")) == NULL)
//        		fclose(fp);
//
//        	int len = fwrite(encbox, 1, encbox_size, fp);
//        	fclose(fp);
//#endif
//        	av_freep(&encbox);
//        	av_freep(&mp4box);
...
#if _SUPPORT_SUMA_DRM // Added by mabiao for sinf atom
static int mov_read_stsd_oma(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    int entries;
...
#if _SUPPORT_SUMA_DRM // added by mabiao for sinf box
...
#if _DRM_TEST //Added by mabiao for taking sample
static int suma_DRM_ProcessSample(const char *pcSample, int *piSampleLen, \
               char *pcOutSample, const char *pcKey, int iKeyLen) {

    if (pcSample[0] == 0x80) {
        for (int i = 17; i < *piSampleLen; i++) {
            pcOutSample[i - 17] = pcSample[i] ^ 0x53;
        }
        *piSampleLen -= 17;
    } else {
        memcpy(pcOutSample, pcSample, *piSampleLen);
    }

    return 0;
}

static int suma_DRM_ProcessSample_mb(uint8_t *pcSample,  int piSampleLen, \
               uint8_t *pcOutSample) {
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    AVStream *st = NULL;
    int ret;
#if _SUPPORT_SUMA_DRM	// Added by mabiao for taking sample
	uint8_t *data_temp;
    int *size_temp;
	int ret_mb;
#if _DRM_TEST	
	static int loop = 0; // Added by mabiao for test
#endif
#endif	
    mov->fc = s;
 retry:
...
#if _DRM_TEST // Added by mabiao for sample test
        data_temp = pkt->data;// sample
        //*size_temp = sample->size;
        ret_mb = suma_DRM_ProcessSample_mb(data_temp, sample->size, pkt->data);//[mabiao]简单sample解密
#endif
        if (ret < 0)
            return ret;
#if _SUPPORT_SUMA_DRM // Added by mabiao for take sample
//        int samplelen = sample->size;
//        ret = suma_drm_process_sample(pkt->data, &samplelen);
//        sample->size = samplelen;
//
//#if _DRM_TEST //Added by mabiao for test
//		loop++;
//		if(200 == loop)
//			ret = 7;
//#endif
...
//			suma_drm_logout(__FILE__, __func__, __LINE__, ret);
//
//			//av_log(NULL, AV_LOG_DRM, "suma_drm_process_sample");	//DRM notify
//			av_log(mov->fc, AV_LOG_DRM, "suma_drm_sample_used_up");	//DRM notify
//
//			//usleep(5000*1000);
//        	//mov->found_mdat = 0;
//        	//goto retry;
